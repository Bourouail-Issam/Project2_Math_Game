...
void GenerateQuizzzQuestions(stQuizz& Quizz) {
	for (short NQuestion = 0; NQuestion < Quizz.NumberQuestion; NQuestion++) {
		Quizz.ListQuestion[NQuestion] = GenerateQuestion(Quizz.QuestionLevel, Quizz.TypeO);
	}
}
...
char OpTypeToSympol(EnOperationType TypeO) {
	switch (TypeO)
	{
	case EnOperationType::Add:
		return '+';
...
void PrintQuesion(stQuizz& Quizz, short NumeroQuestion) {
	cout << "\nQuestion [" << NumeroQuestion << "/" << Quizz.NumberQuestion << "]\n\n";
	cout << Quizz.ListQuestion[NumeroQuestion].Number1 << endl;
	cout << Quizz.ListQuestion[NumeroQuestion].Number2 <<" " << OpTypeToSympol(Quizz.ListQuestion[NumeroQuestion].TypeOperation) << endl;
	cout << "-----------\n";
}
